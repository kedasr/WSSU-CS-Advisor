...
def extract_taken_courses_from_pdf(pdf_path: str) -> set[str]:
    """Extract completed course codes from transcript PDF."""
    reader = PdfReader(pdf_path)
    text = "\n".join((p.extract_text() or "") for p in reader.pages)
    return set(re.findall(r"\bCST\s+\d{4}\b", text))

def load_spring_courses(excel_path: str) -> pd.DataFrame:
...
def is_online_or_async(mode: str) -> bool:
    """Check if course is online or asynchronous."""
    m = (mode or "").lower()
